...
//		qDebug() << "repeat" << currentStatus;
		...
		switch (currentStatus) {
			...
			if(receiveBuffer.contains(SYNC_BYTE)) {
				receiveBuffer = receiveBuffer.right(receiveBuffer.length() - receiveBuffer.indexOf(SYNC_BYTE));
				repeat = true;
		...
		case status::LOOKING_FOR_MSG_CHECKSUM:
			if(receiveBuffer.length() >= int(startOfData + messageSize.value(currentType) + sizeof (quint16)))
			{
				quint16 receivedChecksum;
				memcpy(&receivedChecksum, (receiveBuffer.constData() + startOfData + messageSize.value(currentType)), sizeof (quint16));
                quint16 calculateChecksum = qChecksum(receiveBuffer.constData() + 1, uint(messageSize.value(currentType) + 2 + sizeof (quint32)));
				if(receivedChecksum != calculateChecksum) {
					...
					QByteArray array;
                    array = receiveBuffer.left(int(startOfData + messageSize.value(currentType) + sizeof (quint16)));
